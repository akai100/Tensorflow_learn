...
with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d],
    ...
    with tf.variable_scope('Mixed_7c'):
        ...
        #  给Inception模块中每一条路经声明一个命名空间
        with tf.variable_scope('Branch_0'):
            # 实现一个过滤器边长为１，深度为３２０的卷积层
            branch_0 = slim.conv2d(net, 320, [1, 1], scope='Conv2d_0a_1x1')

        # Inception模块中第二条路径。这条计算路径上的结构本身也是一个Inception结构
        with tf.variable_scope('Branch_1'):
            branch_1 = slim.conv2d(net, 384, [1, 1], scope='Conv2d_0a_1x1')
            # tf.concat寒素可以将多个矩阵拼接起来。tf.concat函数的第一个参数指定
            # 了拼接的维度，这里给出的“3”代表了矩阵是在深度这个维度上进行拼接
        ...
        with tf.variable_scope("Branch_3"):
            ...
            branch_3 = slim.conv2d(
                branch_3, 192, [1, 1], scope='Cov2d_0b_1x1')
