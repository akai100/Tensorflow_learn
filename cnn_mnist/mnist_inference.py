...
# 定义卷积神经网络的前向传播过程。这里添加了一个新的参数
# train，用于区分训练过程和测试过程。这个程序中将用到dropout
# 方法，dropout可以进一步提升模型可靠性并防止过拟合，dropout
# 过程只在训练时使用
def inference(input_tensor, train, regularizer):
    # 声明第一层卷积层的变量并实现前向传播过程。这个过程和6.3.1小节
    # 中介绍的一致。通过使用不同的命名空间来隔离不同层的变量，这可以
    # 让每一层中的变量命名只需要考虑在当前层的作用，而不需要担心重名的问题
    # 。和标准LeNet-5模型不大一样，这里定义的卷积层输入为28*28*1的原始M
    # MNIST图片像素。因为卷积层中使用了全0填充，所以输出维28*28*32的矩阵
    with tf.variable_scope('layer1_conv1'):
        ...
        # 使用边长为5，深度为32的过滤器，过滤器移动的步长为1，并且使用全0填充
        conv1 = tf.nn.conv2d(
    ...
    with tf.variable_scope('layer3-cov2'):
    ...
    # 因为每一层神经网络的输入输出都为一个batch的矩阵，所以这里得到的一个维度也包含了
    # 一个batch中数据的个数。
